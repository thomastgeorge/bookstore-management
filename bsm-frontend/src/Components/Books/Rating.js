...
const RatingComp = ({ text, variant, item }) => {
  ...
  const handleSubmit = () => {
    item.rates = (item.rates || 0) + 1;
    setOpen(false);
  };

  const price = item.price?.toFixed(2) || "0.00";
  const stars = item.avgRating?.toFixed(1) || "0.0";
  const rates = item.rates || 0;
  const type = item.type || "N/A";

  const renderStars = () => {
    const starsArray = [];
    for (let i = 0; i < 5; i++) {
      const starColor = i < Math.floor(item.avgRating) ? "#FFAD33" : "#D1D5DB";
      starsArray.push(
        <svg
          key={i}
          width="16"
